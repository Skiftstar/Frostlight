...
const update = (box) => {
	...
	activeSpeaker.value =
		Audio.speakers.find((speaker) => speaker && speaker.id && speaker.id === Audio.speaker.id)
			.description ?? "Unknown"

	micOptions.value = Audio.microphones.map((microphone) => microphone.description)
	activeMicrophone.value =
		Audio.microphones.find((mic) => mic && mic.id && mic.id === Audio.microphone.id).description ??
		"Unknown"
}
